...
export const fetchHeroInfoData = async (heroId?: string | null) => {
  try {
    ...
    const requestBody = {
      pageSize: 200,
      filters: heroId
        ? [{ field: 'hero_id', operator: 'eq', value: parseInt(heroId) }]
        : [],
      sorts: [{ data: { field: 'hero_id', order: 'desc' }, type: 'sequence' }],
      pageIndex: 1,
      object: [],
      fields: [
        'data.hero._createdAt',
        'data.hero._updatedAt',
        'data.hero.data.name',
        'data.head',
        'data.head_big',
        'data.hero.data.story',
        'data.hero.data.head_big',
...
export const processData = (rawData: RawDataType): Record<string, HeroInfo> => {
  return Object.entries(rawData.data.records).reduce(
    (acc, [key, { data }]) => {
